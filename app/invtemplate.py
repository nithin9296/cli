"""
This module extracts templates for invoice providers
...
class InvoiceTemplate(OrderedDict):
    """
    Represents single template file that lives as .yml files on the disk.
    ...
    parse_number(value)
        Parse number, remove decimal seperator and add other options
    ...
    def __init__(self, *args, **kwargs):
        ...
        #Merge template-sepecific options with defaults
        ...
        #Set issuer if it does not exist.
        if 'issuer' not in self.keys():
            self['issuer'] = self['keywords'][0]

    def prepare_input(self, extracted_str):
        """
        Inout raw string and do transformations as set in template file
    ...
    def matches_input(self, optimized_str):
        """ See if the string matches keywords set in the template file"""
    ...
    def parse_number(self, value):
        assert value.count(self.options['decimal_seperator']) <2,\
            'Decimal seperator cannot be present several times'
        # replace decimal seperator by a  |
        amount_pipe = value.replace(self.options['decimal_seperator'], '|')
        # remove all possible thousand seperator
    ...
    def extract(self, optimized_str):
        ...
        logger.debug('Float parsing: decimal seperator=%s', self.options['decimal_seperator'])
        ...
        for k, v in self['fields'].items():
            if k.startswith('static_'):
                ...
                if k.startswith('sum_amount') and type(v) is list:
                    ...
                    sum_field = True
                #Fields can have multiple expression
                if type(v) is list:
                    res_find = []
                    for v_option in v:
                ...
                if res_find:
                    ...
                    logger.warning("regexp for field %s didnt match", k)
